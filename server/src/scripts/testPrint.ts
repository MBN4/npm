import { printRaw } from '../services/rawPrinter.js';
...
chunks.push(Buffer.from(padBetween('Print Fee:', 'Rs.  1.00', 42) + '\n', 'utf8'));
chunks.push(Buffer.from(padBetween('Discount:', '-Rs.  0.00', 42) + '\n', 'utf8'));
chunks.push(Buffer.from('------------------------------------------\n', 'utf8'));

chunks.push(Buffer.from([0x1B, 0x45, 0x01])); // Bold On
chunks.push(Buffer.from(padBetween('NET TOTAL:', 'Rs. 93.00', 42) + '\n', 'utf8'));
chunks.push(Buffer.from([0x1B, 0x45, 0x00])); // Bold Off
chunks.push(Buffer.from(padBetween('Cash Tendered:', 'Rs. 100.00', 42) + '\n', 'utf8'));
chunks.push(Buffer.from(padBetween('Change Return:', 'Rs.   7.00', 42) + '\n', 'utf8'));
chunks.push(Buffer.from('------------------------------------------\n', 'utf8'));
...
printRaw('Speed-X 400UL', finalBuffer)
